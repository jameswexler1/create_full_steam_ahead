...
- [x] Replaced the coordinate-based phase shortcut with ordered powered-shaft bank indexing, so engines sharing one continuous shaft alternate one-up/one-down through both separated and shared-wall spacing.
...
- [x] `env GRADLE_USER_HOME=/tmp/gradle-home ./gradlew compileJava`, `find src/main/resources -name '*.json' -exec jq empty {} +`, `git diff --check`, and `env GRADLE_USER_HOME=/tmp/gradle-home ./gradlew build` passed on 2026-06-03 after deriving piston animation phase from ordered powered shafts on the shared shaft line.
