...
Boilers expose steam through two port styles: an explicit `boiler_outlet` block or a direct Create
Fluid Pipe connection on an eligible top-layer tank face. Both feed the same pressure-network model.
The removed direct compact engine mode must not be restored: placing a cylinder directly on a boiler
does not couple them, and every engine requires a pipe-fed active `steam_inlet`.
...
The cylinder, piston column, and shaft link remain vertical in v1. Upright and upside-down engines
are pipe-fed and require one active assembled `steam_inlet`.
...
The boiler can be remote from the engine. Steam travels through Create Fluid Pipes from an explicit
outlet or direct boiler pipe port to the cylinder inlet; the engine assembly itself remains vertical.
...
| `piston_head` | `Block + IBE<PistonHeadBlockEntity>` | Physical piston head and engine brain. Validates the stack, consumes inlet steam, and powers the linked shaft. |
...
[Cyl]   [          ]   [Cyl]
          ↑
  one shell position is an active Steam Inlet connected to Create Fluid Pipes
```

Block counts for a minimal engine:
- 14-15 × `steam_cylinder` + 1 active `steam_inlet`, plus up to 1 passive `steam_inlet` for visual symmetry
...
The player builds a Create Fluid Tank structure and heats it with Blaze Burners. Create handles heat indicator, water indicator, water supply tracking, and boiler visuals. The Fluid Tank boiler controller reads its own `BoilerData`, creates stored `steam` from valid active heat and water supply, and tracks pressure even when sealed. Steam ports can be explicit `boiler_outlet` blocks or direct pipes attached to valid top-layer tank faces. They expose the existing Create boiler vessel to Create pipes; they do not create independent steam.
...
The 3×3×2 ring self-assembles visually when all 16 shell positions are filled correctly. A working engine requires one shell position to be the active `steam_inlet`, with an optional second passive `steam_inlet` for symmetry and the remaining shell positions being `steam_cylinder`. A cylinder-only ring may still form as casing while under construction, but it cannot claim or power a shaft. Inlets can occupy any non-shared shell slot. The moment the 16th shell block is placed and the ring is complete:
...
- The root and every shell block cache ring ownership and the selected active inlet
- If no inlet is present, the ring reports `No steam inlet` and remains mechanically incomplete
...
5. Requires one active assembled `steam_inlet` occupying a cylinder shell slot
...
If all mechanical checks pass: the piston head stores references to the cylinder root, active inlet, and shaft. If the player placed a normal Create shaft, it is swapped to `full_steam_ahead:powered_shaft`. The piston/head/linkage visuals assemble around the shaft even when the inlet currently has no delivered steam, matching Create's passive linkage behavior. The engine consumes stored `steam` only through that inlet; zero delivered steam means 0 RPM and 0 SU.
...
- Any `steam_inlet` placed, removed, assembled, or reassigned within the ring
...
Every engine requires a valid active steam inlet occupying one assembled cylinder shell slot.
...
- One cylinder block is designated the ring root (deterministically: lowest BlockPos by Y then X then Z). The root block entity holds ring ownership, active inlet position, and assembled state.
- Implements `IHaveGoggleInformation`: shows ring/inlet status and mirrors the linked engine's pressure, RPM, and SU.
...
- `calculateAddedStressCapacity()`: returns capacity proportional to pressure-limited consumed `steam`.
...
- A ring can contain 0, 1, or 2 inlets while being built, but a working engine requires at least one. At most one inlet is active; the optional second inlet is passive for visual symmetry.
- Blockstate properties:
  - `ASSEMBLED: BooleanProperty` (default false)
...
- The piston head consumes steam exclusively from the active linked inlet. No inlet or no delivered steam means no generated output.
...
src/main/java/dev/gustavo/fullsteamahead/
  ...
  content/
    ...
    piston/
      ...
      PistonHeadBlockEntity.java       ← validates the engine, consumes inlet steam, owns powered shaft
...
- [x] Implement `SteamCylinderBlockEntity` — holds assembled state, ring ownership, inlet link, and root flag
...
- [x] Historical boiler-under-ring detection was implemented, then removed with direct compact mode in Phase 20
- [x] Add goggle overlay: assembly status, active inlet status, linked-engine pressure/RPM/SU
- [x] Add `No steam inlet` goggle state for cylinder-only rings
- [x] Verify: place ring → assembles; remove one block → disassembles; reload preserves assembled state
- [x] Verify negative cases: incomplete ring stays unassembled; blocked center does not create a false ring; cylinder-only ring cannot form a working engine
...
- [x] Upward scan from piston head: one to three piston bodies → one to three empty stroke spaces → horizontal Create shaft; ring and inlet are validated around the piston head
- [x] If valid: store cylinder root, inlet, and shaft refs; power the hidden shaft block entity
...
- [x] Historical direct `BoilerData` engine read was implemented, then removed in Phase 20
- [x] Integrate Full Steam Ahead boiler steam production with Create's `BoilerData.evaluate()`
- [x] Treat a 3x3x1 Fluid Tank boiler as sufficient to produce one full engine's pipe-fed steam budget
...
- [ ] Optional hardening verify: break piston → shaft restores; break inlet → shaft restores; restore/reload → state recovers

Implementation note: the Create compatibility mixin now serves boiler steam production and visuals only. Piston-head engines are not attached boiler devices and never read a boiler directly.

### Phase 5: Steam Fluid and Boiler Outlet

**Goal**: Add storable `steam` fluid and boiler steam ports that generate and pressure-feed steam into Create pipes.
...
- [x] Historical transition verification completed before direct compact mode was removed in Phase 20
...
**Goal**: Let cylinders consume piped `steam` and generate rotation remotely from the boiler room.

- [x] Add `steam_inlet` block, item, block entity, blockstate/model/item model/loot/lang/tags/creative entry
- [x] Allow cylinder-only rings to assemble visually during construction; require 14-15 `steam_cylinder` blocks plus 1 active `steam_inlet` and up to 1 passive visual `steam_inlet` for a working engine
...
- [x] Require one active inlet for engine validation; no valid inlet means no claimed powered shaft
- [x] Add goggle overlays for pipe-fed steam consumption and pressure
...
- [x] Verify: no delivered steam stops output while preserving passive linkage; no inlet prevents engine assembly
...
- [x] Movement rules: `boiler_outlet` attaches to its tank side and output pipe side; the historical cylinder-to-boiler attachment was removed in Phase 20
...
- [x] Verify NBT, kinetic state, steam buffer state, and inlet links survive assembly/disassembly and world reload
...
- [x] Allow a mechanically complete piston/head/ring/inlet/shaft structure to assemble its linkage with no delivered steam; steam availability only controls generated RPM/SU
...
- [x] Require one active assembled `steam_inlet` for inverted validation and never read a boiler directly
- [x] Preserve upright pipe-fed behavior; Phase 20 later made the same inlet requirement universal
...
- [x] Server config for: base capacity, steam production rate, boiler outlet pressure range, and direct boiler pipe output enablement (the obsolete direct compact engine toggle was removed in Phase 20)
...
- [x] Historical direct compact compatibility shortcut was implemented, then removed in Phase 20 so every engine uses the shared pressure model.
...
- [x] Reserve the tall controller's adjacent world cell with an invisible internal controller half.
...
- [x] Implement RPM once in `SteamPhysics.rpm(outputFactor)` so ordinary pipe-fed, admission-controlled, shortage-limited, and leak-limited engines share the same mapping.
...
- [x] Avoid detaching and rebuilding the kinetic source for capacity-only changes; coalesce active RPM ramps into shared 10-tick propagation windows with a `0.5 RPM` accumulated deadband, then apply the exact final target after it settles. Retime established same-direction source trees in place with Create's own conveyed-speed rules, while starts, stops, reversals, source conflicts, ownership changes, SU updates, and animation phase continuity remain immediate.
- [x] Coordinate multiple connected Full Steam Ahead generators through the existing kinetic-network owner: convert every engine's local command into the owner's speed frame, use the strongest compatible command as the shared shaft RPM, suppress fractional follower takeovers, and retain the common command when the owner engine briefly loses steam while other FSA engines remain active. Each engine still contributes only its own generated SU. An all-zero FSA-owned network atomically clears a closed stale source graph instead of allowing its members to drive one another; mixed-mod generators and unresolved external source edges remain Create-owned and preserve legitimate passive rotation.
...
- [x] Reconcile each claimed powered shaft from its piston head after every successful validation,
  including unchanged zero-output reloads, and retry post-load reconciliation against Create's
  independently saved kinetic-network speed while the loaded source graph settles. A stale target,
  capacity, applied command, inherited network snapshot, or circular FSA source chain can no longer
  keep an otherwise steamless shaft powered; externally driven engines still rotate passively.
...
- [ ] Manual test ordinary pipe-fed engines at low, quarter, half, three-quarter, and full output; confirm RPM changes progressively while SU remains proportional.
...
### Phase 20: Remove Direct Compact Engine Mode — Implemented (manual verification pending)

**Goal**: use one coherent steam mass/pressure path for every engine. A cylinder placed directly on
a boiler has no implicit connection; steam must travel through pipes into an active cylinder inlet.

- [x] Require one active assembled `steam_inlet` for upright and inverted engine validation and for
  the shaft placement preview/helper.
- [x] Remove boiler-under-ring scanning and boiler controller references from engine validation.
- [x] Remove piston-head direct `BoilerData` output, burner/water state, synthetic pressure, direct
  source mode, and boiler refresh behavior.
- [x] Remove obsolete boiler caches from cylinder and inlet block entities while safely ignoring old
  `BoilerPos` NBT left in existing worlds.
- [x] Migrate old `DIRECT_BOILER` piston-head save data to zero output. On server revalidation, a
  legacy inlet-less engine releases its hidden powered shaft back to a normal Create shaft.
- [x] Remove `balance.enableDirectCompactMode` from the server config.
- [x] Remove the special Create/Simulated movement attachment between an upright cylinder shell and
  a Fluid Tank below it.
- [x] Preserve explicit `boiler_outlet` blocks, direct boiler-to-pipe ports, sealed-boiler pressure,
  boiler displays, pressure gauges, relief valves, and all other boiler physics.
- [x] Keep cylinder-only rings visually constructible, but report `No steam inlet` and prevent them
  from claiming or powering a shaft.
- [x] Update active design, contributor, and verification documentation to the pipe-fed-only rule.
- [ ] Manual migration test: load an old direct compact engine and confirm its shaft becomes a normal
  stationary Create shaft without item loss or a crash.
- [ ] Manual functional test: add an inlet and pipe supply to that ring, replace/preview the shaft,
  and confirm normal pressure-driven RPM/SU, exhaust, sound, and animation.
- [ ] Manual orientation test: verify upright and inverted engines both reject inlet-less rings and
  both run through active inlets.
- [ ] Manual movement test: verify a pipe-fed engine and its inlet/pipe connection survive a
  Simulated/Aeronautics assembly cycle without pulling in an unrelated Fluid Tank below the ring.
