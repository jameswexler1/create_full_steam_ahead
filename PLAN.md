...
- [x] Explosion power = `min(maxPower, basePower + powerPerVolume · networkVolume) · powerScale`, with block-breaking configurable. Defaults are `basePower=12.0`, `powerPerVolume=0.45`, `maxPower=36.0`, `powerScale=0.75`.
- [x] Create Big Cannons projectiles that hit a Create Fluid Tank belonging to an active pressurized steam boiler network trigger a boiler rupture. The rupture reuses the normal burst path, scales power by current pressure relative to burst pressure, and depressurizes the connected steam network.
